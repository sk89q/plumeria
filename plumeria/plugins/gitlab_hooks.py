...
async def url(message):
    """
    Get the webhook URL to use. This URL is to be placed under the "Webhooks" section of GitLab teams
    or projects. To actually enable the hook, a token must be generated and added with
    the token manage commands.
    """
...
async def add_token(message):
    ...
    and stored with the webhook when it is added. By default, the GitLab hooks only process
    webhook requests containing the given token. Tokens are per-server.
    """
