"""Get similar music, movies, shows, books, authors, and games using tastekid.com."""
...
def make_command(type, names, example):
    @commands.create(*map(lambda s: "similar {}".format(s), names), category="Search")
    @rate_limit()
...
def setup():
    ...
    if not api_key():
        raise PluginSetupError("This plugin requires an API key from https://tastekid.com. Registration is free.")

    for e in types:
        commands.add(make_command(*e))
